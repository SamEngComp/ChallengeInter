...
class RemoteFetchAlbumsTests: XCTestCase {
    ...
    func test_fetch_should_complete_with_albums_if_client_completes_with_valid_data() {
        let url = URL(string: "http://fake-url-test.com")!
        let httpClientSpy = HttpClientSpy()
        let sut = RemoteFetchAlbums(url: url, httpGetClient: httpClientSpy)
        let albumsSpy = [Album(id: 12, userId: 100, title: "title")]
        let exp = expectation(description: "waiting")
        sut.fetch(userId: 0) { result in
            switch result {
            case .success(let albums):
                XCTAssertEqual(albums, albumsSpy)
            case .failure(_):
                XCTFail()
            }
            exp.fulfill()
        }
        if let data = try? JSONEncoder().encode(albumsSpy.self) {
